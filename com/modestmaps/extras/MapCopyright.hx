...
{
	private static inline var script_js:String = "<script>
			<![CDATA[
			function() {
			modestMaps = {
	...
	private static var scriptAdded:Bool = false;
	...
	public function new(map:Map, offsetX:Float=10, offsetY:Float=10)
	{
		...
		if (!scriptAdded)
		{
			...
			{
				trace("problem adding setCopyright as callback in Map.hx");
	...
	private var copyrightTimeout:UInt;
	...
	*/
	private function callCopyright():Void
	{
		...
		var minLat:Float = Math.min(minL.lat, maxL.lat);
		var minLon:Float = Math.min(minL.lon, maxL.lon);
		var maxLat:Float = Math.max(minL.lat, maxL.lat);
		var maxLon:Float = Math.max(minL.lon, maxL.lon);
	...
	*/
	public function setCopyright(copyright:String):Void
	{
		...
		StringTools.replace(this.copyright,"/&copy;/g", "©");
