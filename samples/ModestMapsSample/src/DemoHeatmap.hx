...
class DemoHeatmap
{
	private var overlay : HeatMarkerClip;
	private var map : TweenMap;

	public function new(map : TweenMap) 
	{
		this.map = map;
        map.addEventListener(MapEvent.STOP_ZOOMING, onZoomEnd);
		map.tweenExtent(new MapExtent(41.5, 41.3, 2, 2.3),2);
        
	}

	private function onZoomEnd(e: Event) : Void 
	{
		map.removeEventListener(MapEvent.STOP_ZOOMING, onZoomEnd);
		...
		overlay = new HeatMarkerClip(map, locations, HeatMarkerClip.THERMAL, 10, map.getZoom());
...
class HeatMarkerClip extends Sprite
{
    ...
    private var gradientArray : Array<Int>;
    ...
    public function new(map : TweenMap, locations : Array<Location>, gradientArray : Array<Float>, nativeRadius : Int = 25, nativeZoom : Int = 11)
    {
        super();
        buttonMode = false;
        mouseEnabled = false;
		...
		this.gradientArray = new Array<Int>();
		for (i in gradientArray) {
			this.gradientArray.push(Std.int(i));
		}
    ...
    private function drawHeatMap(event : MapEvent = null) : Void
    {
        ...
        itemRadius = Std.int(Math.pow(2, map.getZoom() - nativeZoom)) * nativeRadius;
        
		if (itemRadius < 5) itemRadius = 5;
		//if (itemRadius > 20) itemRadius = 20;
		//if (itemRadius = 0) itemRadius = 1;
        ...
        var heatMapItem : BitmapData = new BitmapData(Std.int(heatMapShape.width), Std.int(heatMapShape.height), true, 0x00000000);
        ...
        if (heatBitmapData == null || heatBitmapData.width != w || heatBitmapData.height != h) {
            ...
            heatBitmapData = new BitmapData(Std.int(w), Std.int(h), true, 0x00000000);
